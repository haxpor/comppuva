...
public:
    bool hasCycle(ListNode* head) {
        ...
        std::function<bool(const ListNode* node, std::unordered_set<const ListNode*>& visited)> dfsCycle;

        // return true if found cycle
        dfsCycle = [&dfsCycle](const ListNode* node, std::unordered_set<const ListNode*>& visited) -> bool {
            if (node == nullptr)
                return false;
